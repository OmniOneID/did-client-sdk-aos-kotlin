...
class RestoreUser {
    ...
    fun restoreUser(
        ...
        val api5 = "/tas/api/v1/request-restore-diddoc"
        ...
        return CompletableFuture.supplyAsync {
            runCatching {
                httpUrlConnection.send(
                    tasUrl + api5,
                    "POST",
                    M142_RequestRestoreDidDocByWallet(txId, serverToken, didAuth)
                )
            }.getOrElse { e ->
                when (e) {
    ...
    private fun M142_RequestRestoreDidDocByWallet(
        txId: String,
        serverToken: String,
        didAuth: DIDAuth
    ): String {
        val requestVo = P142RequestVo(WalletUtil.createMessageId(), txId).apply {
